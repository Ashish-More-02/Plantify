import React from "react";

const About = () => {
  return (
    <div className="w-full">
      <h1 className="text-5xl font-semibold text-center mt-28 mb-12">
        About Plantify
      </h1>

      <div className="w-full">
        <div className="bg-[rgb(251,246,233)] sm:w-[80%]  mx-6 sm:mx-auto text-2xl sm:text-4xl font-semibold text-center py-10 rounded-3xl my-5 ">
          🌱 Your one-stop destination for all things green and beautiful.
        </div>
        <div className="bg-white sm:w-[80%] mx-6 sm:mx-auto text-2xl sm:text-4xl font-semibold text-center py-10 rounded-3xl my-5 ">
          🌿 We believe plants are more than decor—they're life-givers and
          stress-busters.
        </div>
        <div className="bg-[rgb(251,246,233)] sm:w-[80%]  mx-6 sm:mx-auto text-2xl sm:text-4xl font-semibold text-center py-10 rounded-3xl my-5 ">
          🍀 Offering air-purifying plants, gardening tools, and everything to
          create your paradise.
        </div>
        <div className="bg-white sm:w-[80%]  mx-6 sm:mx-auto text-2xl sm:text-4xl font-semibold text-center py-10 rounded-3xl my-5 ">
          🌻 Perfect for both seasoned gardeners and beginners.
        </div>
        <div className="bg-[rgb(251,246,233)] sm:w-[80%]  mx-6 sm:mx-auto text-2xl sm:text-4xl font-semibold text-center py-10 rounded-3xl my-5 ">
          🌍 Providing carefully curated plants, eco-friendly products, and
          expert guidance.
        </div>
        <div className="bg-white sm:w-[80%]  mx-6 sm:mx-auto text-2xl sm:text-4xl font-semibold text-center py-10 rounded-3xl my-5 ">
          🏡 On a mission to bring nature closer to every home, workplace, and
          community.
        </div>
        <div className="bg-[rgb(251,246,233)] sm:w-[80%]  mx-6 sm:mx-auto text-2xl sm:text-4xl font-semibold text-center py-10 rounded-3xl my-5 ">
          🌳 Join us to grow a greener, healthier future together!
        </div>
      </div>
    </div>
  );
};

export default About;
